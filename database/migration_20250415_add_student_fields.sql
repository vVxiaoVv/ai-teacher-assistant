...
ON DUPLICATE KEY UPDATE
    ...
    `age` = VALUES(`age`),
    `exam_history` = VALUES(`exam_history`),
    `update_time` = NOW();
...
-- 4. 验证数据
-- ============================================
SELECT '=== 学生画像表结构 ===' AS info;
DESCRIBE `student_portrait`;

SELECT '=== 学生画像数据统计 ===' AS info;
SELECT 
    COUNT(*) AS total_count,
    COUNT(`age`) AS age_filled_count,
    COUNT(`exam_history`) AS exam_history_filled_count
FROM `student_portrait`;

SELECT '=== 学生画像详细数据 ===' AS info;
SELECT 
    `id`,
    `student_name`,
    `age`,
